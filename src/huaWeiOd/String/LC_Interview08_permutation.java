    ...
    public void backtrack(int index) {
        if (index == n) {
            ...
            for (int i = 0; i < n; i++) {// 从0开始！！！
                if (visited[i] ||   (i > 0 && arr[i] == arr[i - 1] && !visited[i - 1])    ) { // 当前字符已加入过当前排列  或 该字符和前个相同，且前个还没访问过（如qqe的第2个q） 都不能将i位置元素加入当前排列；  （注意i>0 ,才能保证i-1>0）
                    continue;
                }
